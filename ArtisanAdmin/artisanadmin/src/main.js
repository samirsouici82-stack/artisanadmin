...
import {
  ...
  getEmailUtilisateurConnecte
} from './lib/supabase-client.js';

const PLAN_LABELS = { essentiel: 'Essentiel (29€/mois)', pro: 'Pro (49€/mois)' };
let pendingPlan = null; // plan choisi sur la vitrine avant inscription/connexion
...
function renderSubscription(ctx) {
  const ab = ctx.abonnement;
  ...
  if (ab.statut === 'essai') {
    const jours = joursRestants(ab.periode_fin);
    el.innerHTML = `
      <div class="sub-card trial">
        <div>
          <div class="sub-status">Essai gratuit</div>
          <div class="sub-title">${jours} jour${jours > 1 ? 's' : ''} restant${jours > 1 ? 's' : ''}</div>
          <div class="sub-desc">Passez à un abonnement payant à tout moment, aucune carte n'est requise pendant l'essai.</div>
        </div>
        <div class="sub-actions">
          <button class="btn-ghost" data-plan="essentiel" id="btnSubEssentiel">Essentiel — 29€/mois</button>
          <button class="btn-primary" data-plan="pro" id="btnSubPro">Pro — 49€/mois</button>
        </div>
      </div>`;
    ...
    el.innerHTML = `
      <div class="sub-card">
        <div>
          <div class="sub-status">Abonnement actif</div>
          <div class="sub-title">${PLAN_LABELS[ab.plan] || ab.plan}</div>
          <div class="sub-desc">Facturation automatique chaque mois.</div>
        </div>
        <div class="sub-actions">
          <button class="btn-ghost" id="btnGererAbonnement">Gérer mon abonnement</button>
        </div>
      </div>`;
  } else {
    // 'impaye' ou 'annule'
    el.innerHTML = `
      <div class="sub-card impaye">
        <div>
          <div class="sub-status">${ab.statut === 'impaye' ? 'Paiement en échec' : 'Abonnement résilié'}</div>
          <div class="sub-title">Accès limité</div>
          <div class="sub-desc">Mettez à jour votre moyen de paiement pour retrouver un accès complet.</div>
        </div>
        <div class="sub-actions">
          <button class="btn-primary" id="btnGererAbonnement">Régulariser mon abonnement</button>
        </div>
      </div>`;
  }

  el.querySelectorAll('[data-plan]').forEach((btn) => {
    btn.addEventListener('click', () => lancerPaiement(btn.dataset.plan));
  });
  const btnGerer = $('btnGererAbonnement');
  if (btnGerer) btnGerer.addEventListener('click', gererAbonnement);
}

async function lancerPaiement(plan) {
  try {
    showToast('Redirection vers la page de paiement…');
    const email = await getEmailUtilisateurConnecte();
    const url = await demarrerAbonnement({
      artisanId: contexteActuel.profil.id,
      email,
      plan
    });
    window.location.href = url;
  } catch (err) {
    showToast("Impossible de démarrer le paiement : " + err.message);
  }
}

async function gererAbonnement() {
  try {
    const url = await ouvrirPortailAbonnement(contexteActuel.abonnement.stripe_customer_id);
    window.location.href = url;
  } catch (err) {
    showToast("Impossible d'ouvrir le portail : " + err.message);
  }
}
...
function wireEvents() {
  ...
  document.querySelectorAll('.btn-go-signup').forEach((btn) => {
    btn.addEventListener('click', () => {
      pendingPlan = btn.dataset.plan || null;
      showLogin('signup');
    });
  });
